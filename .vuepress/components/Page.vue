  ...
  <b-row class="p-0 pt-4">
    ...
    <b-col v-if="loaded" lg="8" offset-lg="2" class="mt-2 p-0">
      <b-link :href="sctLink" class="text-dark text-decoration-none">
        <b-card class="mb-4" role="button" bg-variant="warning">
          <b-row>
            <b-col sm="2">
              <b-img
                :src="$withBase(`/assets/images/token-watcher.svg`)"
                fluid></b-img>
            </b-col>
            <b-col sm="10">
          ...
          <template #footer>
            Want to try new features? We created <b>SmartContracts Tools</b> at
            <b>NONCEPT.</b>
          </template>
        </b-card>
      </b-link>
      ...
      <b-jumbotron bg-variant="light" header-level="4" class="px-3 py-4">
        ...
        <hr class="my-4" />
        <h6>
          Name: <span class="text-muted">{{ token.name }}</span>
        </h6>
        <h6>
          Symbol: <span class="text-muted">{{ token.symbol }}</span>
        </h6>
        <h6>
          Decimals: <span class="text-muted">{{ token.decimals }}</span>
        </h6>
        <h6>
          Address:
  ...
  computed: {
    sctLink() {
      const network = this.network.current
        ? this.network.current.blockchain === 'Ethereum'
          ? `ethereum_${this.currentNetwork}`
          : this.currentNetwork
        : 'ethereum_mainnet';
  ...
  methods: {
    ...
    async shareToken() {
      this.share.tokenLink =
        ...
        this.$withBase(
          `/page/?hash=${this.token.hash}&network=${this.currentNetwork}`,
        );

      this.share.shortLink = await this.shorten(this.share.tokenLink);

      this.share.facebook = `https://www.facebook.com/sharer.php?u=${this.share.shortLink}&quote=Discover more about ${this.token.name} (${this.token.symbol}).`;
      this.share.twitter = `https://twitter.com/intent/tweet?url=${this.share.shortLink}&text=Discover more about ${this.token.name} (${this.token.symbol}).`;
      this.share.telegram = `https://t.me/share/url?url=${this.share.shortLink}&text=Discover more about ${this.token.name} (${this.token.symbol}).`;
      this.share.whatsapp = `https://wa.me/?text=Discover more about ${this.token.name} (${this.token.symbol}). ${this.share.shortLink}`;
